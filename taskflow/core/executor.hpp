...
void Executor::_corun_until(Worker& w, P&& stop_predicate) {
  ...
  while(!stop_predicate()) {
    ...
    auto t = task_arena_ptr.get() == nullptr ? w._wsq.pop() : w._task_arena_ptr->_wsq_per_worker_id[w._id].pop();
    if(t) {
      _invoke(w, t);
    }
    else {
      ...
      explore:
        ...
        if (w._id == w._vtm && task_arena_ptr.get() == nullptr) {
          ...
          std::shared_ptr<TaskArena> vtm_task_arena_ptr;
          {
            std::shared_lock<std::shared_mutex> lock(_workers_mutex);
            vtm_task_arena_ptr = _workers[w._vtm]._task_arena_ptr;
          }
...
inline void Executor::_explore_task(Worker& w, Node*& t) {
  ...
  do { 
    ...
    if (w._id == w._vtm && task_arena_ptr.get() == nullptr) {
      ...
      if (!task_arena_ptr->_wsq_per_worker_id.empty())
        t = task_arena_ptr->_wsq_per_worker_id[w._vtm].steal();
    }
